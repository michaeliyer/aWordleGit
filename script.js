...
//     } else {
//       const notFoundString = `The word "${inputWord}" was not found. Shut up, but<br> feel free, feel obligated, to guess the word "${inputWord}", Fuckface!`;
...
//     const listWithTotal = `${wordsList} (${totalWords} Christing words exist in this frickin list! It's steadily shrinking...)`;
...
//   document.getElementById("countdownToMidnight").textContent = `Countdown to Midnight: 🧌 ${getTimeUntilMidnightEastern()} The Clock Can't Tick Fast Enough!`;
// }
...
//   if (countdownElement) {
//     countdownElement.textContent = `The American Mid-Term Elections 2026 Are In  ${calculateCountdown("2026-11-03T00:00:00")}. 
//     Can We Fucking Make It Till Then, you dastardly fools? Likely we're fucked!`;
//   }
// }
...
function updateFooter() {
  ...
  // Update countdown to midnight
  document.getElementById("countdownToMidnight").textContent = `Countdown to Midnight: 🧌 ${getTimeUntilMidnightEastern()} Clocks Simply Can't Tick Fast Enough!`;
}
...
function calculateCountdown(targetDate) {
  ...
  return `${days} very longAss fickin' days, ${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`;
}
