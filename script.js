...
function handleWordInput() {
  ...
  if (foundWord) {
    ...
    const resultString = `'${foundWord.word}': That WORD was ALREADY Goddamn FUCKING employed by this Pissing Wordle affair (as I've said! Repeatedly! Listen!) on ${foundWord.gameDate}. Goddamn you Ace! <br> It was word #${foundWord.wordNumber}, 
          and ye had ye a score of '${foundWord.myScore}'!<br> Your Score Average thru this date: ${averageScoreUpToDate}!<br> Do NOT guess '${foundWord.word}', and do not waste our time!`;
    document.querySelector(".field-one").innerHTML = resultString;
  } else {
    const notFoundString = `The word "${inputWord}" was not found, wretched Whores, making Rand Paul that rapist. Hit that fuck again neighbor. Shut up, but<br> feel free, feel obligated, to guess the word "${inputWord}", Fuckface!`;
...
function displayResults(results) {
  ...
  const listWithTotal = `${wordsList} (${totalWords} Christing idiotic stupidAssHoleWords existParasitically in this here unlit motherfrickin' list, assholes!! It's steadily shrinking, dickLicks, like my sanity, DICKLORD!)`;
...
function applyFilter(selectedFunction, letter, position) {
  switch (selectedFunction) {
    ...
    case "containsDuplicateLetters":
      ...
      break;
    default:
      alert("Please select a valid function, you stupid, stupid asshole.");
      return;
  }
}
...
calculateAverageButton.addEventListener("click", () => {
  ...
  averageScoreResult.textContent = `Your Average Score - (You smell like shit, Cocklick!) - between words #${start} & #${end} = a gd, a-f-ing 'damn crispAss shitting ${averageScore.toFixed(
    6
  )}, You Cocksucker!`;
});
