...
function handleWordInput() {
  ...
  if (foundWord) {
    ...
    const resultString = `'${foundWord.word}': That, um, most retardally, um, idiotical WORD was so abso-fuckin-um-lutely fuckin' ALREADY Goddamn FUCKING em-fuckin'-ployed by this Pissing Ass Wordle affair (as I've said! Repeatedly! Listen!) on ${foundWord.gameDate}. Goddamn you Ace! <br> It was word #${foundWord.wordNumber}, 
          and ye had ye a score of '${foundWord.myScore}'!<br> Your Score Average thru this here stupid ass lousy, um, fuckin' date: ${averageScoreUpToDate}!<br> Do NOT guess '${foundWord.word}', and do not waste our time!`;
    document.querySelector(".field-one").innerHTML = resultString;
  } else {
    const notFoundString = `The fockin word "${inputWord}", which is an innocent, ah, but extremely foolosh bystander †‡ †‡ †‡ , was notteth found, oh wretched Whores and Assfucks, making Rand Paul that rapist. Hit that fuck again neighbor. Shut up, but<br> feel free, feel obligated, to guess the word "${inputWord}", Fuckface! You're <span><em>all</em></span> fuckfaces.`;
...
function displayResults(results) {
  ...
  const listWithTotal = `${wordsList} (${totalWords} Christing †‡ and dick yeah, um, oh ho, gagging, um, grossly, um, outwardly, blatantly, fantastically idiotical and, um, goddamn cunting †‡ stupid ass frickin' fuckin' AssHole 🧌 🧌 🧌  Fuckin' Words stupidestly exist stupidly and Parasitically, um, in this here unlit motherfrickin' list, assholes!! It's steadily shrinking, dickLicks, like my sanity, DICKLORD!)`;
...
function applyFilter(selectedFunction, letter, position) {
  switch (selectedFunction) {
    ...
    case "containsDuplicateLetters":
      ...
      break;
    default:
      alert("Please select a valid function, you goddamn, um, stupid, stupid asshole.");
      return;
  }
}
...
calculateAverageButton.addEventListener("click", () => {
  ...
  averageScoreResult.textContent = `Your Average Score - (You smell really, um, much and, um, continuously, um, like cockshit, Cocklick! Um, Cocklord! Cumlick!) - betweeneth words #${start} & #${end} = a gd, a-f-ing 'damn crisp-ass dick-shitting ${averageScore.toFixed(
    6
